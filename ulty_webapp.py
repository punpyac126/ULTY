...
if mode == "📍 อยากได้ปันผลเท่าไหร่":
    ...
    if div_mode == "🪙 รายสัปดาห์":
        weekly_input = st.text_input("💸 ปันผลที่ต้องการต่อสัปดาห์ (หลังหักภาษี)", value="5,000.00")
        weekly_div_net = parse_comma_input(weekly_input, default=5000.0)
    else:
        monthly_input = st.text_input("📆 ปันผลที่ต้องการต่อเดือน (หลังหักภาษี)", value="20,000.00")
    ...
    if stock_price and exchange_rate:
        shares_needed, total_investment, monthly_gross = calculate_required_investment(
            weekly_div_net, stock_price, exchange_rate
        )
        st.subheader("📊 ผลลัพธ์")
        st.markdown(f"- จำนวนหุ้นที่ต้องซื้อ: **{shares_needed:,} หุ้น**")
        st.markdown(f"- เงินลงทุนรวม: **{total_investment:,.2f} บาท**")
        st.markdown(f"- ปันผลต่อเดือน (ก่อนหักภาษี): **{monthly_gross:,.2f} บาท**")
        st.markdown(f"- ปันผลต่อเดือน (หลังหักภาษี): **{weekly_div_net * 4:,.2f} บาท**")
    else:
